...
class SchemaCreatorStandalone:
    ...
    def create_silver_schema(self):
        """Create Silver layer main table schema - simplified"""
        if not self.spark:
            return False
        ...
        try:
            ...
            # Only create oer_resources table - the main Silver table
            # Other tables (oer_subjects, oer_relations, etc.) are not used
            # and have been removed to simplify the schema
            
            print(" Silver layer schema ready (uses oer_resources from silver_transform.py)")
            return True
